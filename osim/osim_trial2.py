        ...
        def is_valid_state() -> bool:
            try:
                for i in range(self.muscleSet.getSize()):
                    ...
                    # Confirm it's a Muscle before calling getFiberLength
                    if not Muscle.safeDownCast(force):
                        continue
                ...
                print(f"[is_valid_state] Exception: {e}")
                return False
    ...
    def step(self, actions: np.ndarray) -> tuple[np.ndarray, float, bool, bool, dict]:
        ...
        try:
            ...
            for name in self.critical_muscles:
                if not self.forceSet.contains(name):
                    print(f"[WARN] {name} not found in forceSet")
                    continue

                force = self.forceSet.get(name)
                muscle = Muscle.safeDownCast(force)

                if muscle is None:
                    print(f"[WARN] {name} is not a Muscle, skipping")
                    continue

                fiber_len = muscle.getFiberLength(self.state)

                if fiber_len <= 0.001:
                    raise ValueError(f"Fiber length too short for {name} ({fiber_len:.5f})")
        except Exception as e:
